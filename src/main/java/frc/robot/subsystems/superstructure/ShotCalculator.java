...
public class ShotCalculator {
    ...
    public record HoodCalculation(
            Rotation2d hoodRotation,
            double flywheelVelocity,
            double shotTime
    )  implements Interpolatable<HoodCalculation> {
        public static final Interpolator<HoodCalculation> interpolator = HoodCalculation::interpolate;


        @Override
        public HoodCalculation interpolate(final HoodCalculation endShotCalculation, final double t) {
            return new HoodCalculation(
    ...
    private static final InterpolatingTreeMap<Double, HoodCalculation> shotDataMap = new InterpolatingTreeMap<>(
            InverseInterpolator.forDouble(),
            HoodCalculation.interpolator
    );

    static {
        shotDataMap.put(1d, new HoodCalculation(
                Rotation2d.fromRadians(0.437138669),
                10,
                0.1
        ));

        shotDataMap.put(1.5d, new HoodCalculation(
                Rotation2d.fromRadians(0.611352924),
                10,
                0.15
        ));

        shotDataMap.put(2d, new HoodCalculation(
                Rotation2d.fromRadians(0.75159462),
                10,
                0.2
        ));

        shotDataMap.put(2.5d, new HoodCalculation(
                Rotation2d.fromRadians(0.86282925),
                10,
                0.20
        ));

        shotDataMap.put(3d, new HoodCalculation(
                Rotation2d.fromRadians(0.951177756),
                10,
                0.25
        ));

        shotDataMap.put(3.5d, new HoodCalculation(
                Rotation2d.fromRadians(1.022015817),
                10,
                0.3
        ));

        shotDataMap.put(4d, new HoodCalculation(
                Rotation2d.fromRadians(1.079542321),
                10,
                0.35
        ));

        shotDataMap.put(4.5d, new HoodCalculation(
                Rotation2d.fromRadians(1.126894805),
                10,
                0.4
        ));

        shotDataMap.put(5d, new HoodCalculation(
    ...
    public record ShotCalculation(
            ...
            HoodCalculation hoodCalculation
    ) {}
